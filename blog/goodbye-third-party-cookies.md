...
Of course, first-party cookies can *also* be used to track your activity, as with Google Analytics, but they can't be used to follow you beyond that site. While both types of cookies can be used to track users, third-party cookies are much more invasive and problematic.
...
That said, I have no issue with the use of first-party cookies, provided their use is consensual and intended to improve the user experience. I understand their importance for many organizations, especially in marketing.
...
2. Complex digital platforms, like healthcare and government websites, sometimes use cross-site authentication to link departmental websites. Blocking these cookies could prevent access to important services.

While Safari and Firefox have been limiting third-party cookies for some time, Google Chrome is lagging behind. Google only began phasing out third-party cookies in early 2024 (a few weeks ago), starting with just 1% of its users. Its plan is to expand this to all users by the end of 2024.
...
2. Positively, Google is cautious about removing third-party cookies to avoid disrupting websites that rely on them for non-advertising purposes.

As a regular Chrome user, I didn't want to wait until the end of 2024. If you feel the same, you can block third-party cookies in Chrome now. Just head to `Settings`, select `Privacy and Security` and activate `Block third-party cookies`. Just be aware that some sites might stop working.

If you manage or develop websites, check whether they rely on third-party cookies. Use [my HTTP Header Analyzer](https://dri.es/the-little-http-header-analyzer-that-could) to check for `SameSite=None` attributes indicating third-party cookies, or test by disabling them in Chrome.
