---
url: 'https://dri.es/the-business-behind-open-source'
title: 'The business behind Open Source'
author:
  name: 'Dries Buytaert'
  url: 'https://dri.es/about'
date: '2014-07-24T10:38:13-04:00'
license: 'https://creativecommons.org/licenses/by/4.0/'
type: blog
summary: 'Notes from a conversation with The New York Times on how Open Source can build high-growth businesses while helping the community thrive.'
tags:
  - Drupal
  - Acquia
  - 'Open Source'
  - 'Business model'
  - 'Open Source sustainability'
  - Press
published: true
featured: false
id: 3181
---

# The business behind Open Source

A few days ago, I sat down with [Quentin Hardy](https://www.linkedin.com/in/qhardy/) of [The New York Times](http://nytimes.com) to talk Open Source. We spoke mostly about the [Drupal ecosystem](https://www.drupal.org) and how [Acquia](https://www.acquia.com) makes money. As someone who has spent almost his entire career in Open Source, I'm a firm believer in the fact that you can [build a high-growth, high-margin business and help the community flourish](https://dri.es/do-well-do-good). It's not an either-or proposition, and Acquia and Drupal are proof of that.

Rather than being a [utopian alternate reality](https://bits.blogs.nytimes.com/2014/07/23/open-source-and-the-challenge-of-making-money/), as Quentin outlines, I believe Open Source is both a better way to build software and a good foundation for an ecosystem of for-profit companies. Open Source software itself is very successful and is capable of running some of the most complex enterprise systems. But failure to commercialize Open Source doesn't necessarily make it bad.

I mentioned to Quentin that I thought Open Source was [Darwinian](https://en.wikipedia.org/wiki/Darwinism); a proprietary software company can't afford to experiment with creating 10 different implementations of an online photo album, only to pick the best one. In Open Source, we can and do. We often have competing implementations, and eventually the best implementation(s) will win. One could say that Open Source is a more "wasteful" way to develop software. In a pure capitalist reading of [On the Origin of Species](https://en.wikipedia.org/wiki/On_the_Origin_of_Species), there is only one winner, but business and Darwin's theory itself are far more complex. Beyond "only the strongest survive", Darwin tells a story of interconnectedness, or the way an ecosystem can dictate how an entire species chooses to adapt.

While it's true that the Open Source "business model" has produced few large businesses ([Red Hat being one notable example](http://quotes.morningstar.com/stock/RHT/s?t=RHT)), we're also evolving the different Open Source business models. In the case of Acquia, we're selling a number of "as-a-service" products for Drupal, which is vastly different than just selling support like the first generation of Open Source companies did.

As a private company, [Acquia](https://www.acquia.com) doesn't disclose financial information, but I can say that we've been very busy operating a high-growth business. Acquia is North America's fastest-growing private company on the Deloitte Fast 500 list. Our Q1 2014 bookings increased 55 percent year-over-year, and the majority of that is recurring subscription revenue. We've experienced 21 consecutive quarters of revenue growth, with no signs of slowing down. Acquia's business model has been both disruptive and transformative in our industry. Other Open Source companies like [Hortonworks](http://hortonworks.com), [Cloudera](http://cloudera.com) and [MongoDB](http://mongodb.com) seem to be building thriving businesses too.

Society is undergoing tremendous change right now – the sharing and collaboration practices of the internet are extending to transportation ([Uber](http://uber.com)), hotels ([Airbnb](http://airbnb.com)), financing ([Kickstarter](http://kickstarter.com), [LendingClub](http://lendingclub.com)) and music services ([Spotify](http://spotify.com)). The [rise of the collaborative economy](https://blogs.wsj.com/accelerators/2014/04/23/jeremiah-owyang-the-collaborative-economy-will-double-this-year/), of which the Open Source community is a part, should be a powerful message for the business community. It is the established, proprietary vendors whose business models are at risk, and not the other way around.

Hundreds of other companies, including several venture-backed startups, have been born out of the Drupal community. Like Acquia, they have grown their businesses while supporting the ecosystem from which they came. That is more than a feel-good story; it's just good business.
